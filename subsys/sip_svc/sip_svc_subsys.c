...
static uint32_t sip_svc_get_c_idx(struct sip_svc_controller *ctrl, uint32_t c_token)
{
	...
	for (i = 0; i < CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT; i++) {
			     ...
			     struct sip_svc_response *response)
{
	...
	__ASSERT(c_idx < CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT,
		 "c_idx shouldn't be greater than CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT");
...
static int sip_svc_request_handler(struct sip_svc_controller *ctrl)
{
	...
	/**
	 * If transaction's are more than CONFIG_ARM_SIP_SVC_SUBSYS_MAX_JOB_COUNT,
	 * return -EBUSY.
	 */
	if (ctrl->active_job_cnt >= CONFIG_ARM_SIP_SVC_SUBSYS_MAX_JOB_COUNT) {
		return -EBUSY;
	}
...
static int sip_svc_async_response_handler(struct sip_svc_controller *ctrl)
{
	...
	uint32_t trans_id;
	uint64_t data_addr;
	size_t data_size = CONFIG_ARM_SIP_SVC_SUBSYS_MAX_ASYNC_RESP_SIZE;
	int ret;
...
static int sip_svc_subsys_init(void)
{
	...
	STRUCT_SECTION_FOREACH(sip_svc_controller, ctrl)
	{
		...
		ctrl->client_id_pool =
			sip_svc_id_mgr_create(CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT);
		if (!ctrl->client_id_pool) {
			return -ENOMEM;
		}

		ctrl->trans_id_map = sip_svc_id_map_create(CONFIG_ARM_SIP_SVC_SUBSYS_MSGQ_DEPTH);
		...
		for (uint32_t i = 0; i < CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT; i++) {
			...
			client->trans_idx_pool = sip_svc_id_mgr_create(
				CONFIG_ARM_SIP_SVC_SUBSYS_MAX_TRANSACTION_COUNT);
