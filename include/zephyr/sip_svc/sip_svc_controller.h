...
struct sip_svc_controller {
	...
	/* Array of total number of clients supported */
	struct sip_svc_client clients[CONFIG_ARM_SIP_SVC_SUBSYS_MAX_CLIENT_COUNT];
	/* Stack object of sip_svc thread */
	K_KERNEL_STACK_MEMBER(stack, CONFIG_ARM_SIP_SVC_SUBSYS_THREAD_STACK_SIZE);
	/* Buffer used for storing response from lower layers */
	uint8_t async_resp_data[CONFIG_ARM_SIP_SVC_SUBSYS_MAX_ASYNC_RESP_SIZE];
};
 ...
 */
#define SIP_SVC_CONTROLLER_DEFINE(inst, conduit_name, sip_dev)                                     \
